...
@section('content')
    <div class="mb-4">
        <h1 class="text-warning"><i class="bi bi-plus-circle me-2"></i>Cadastrar Parada</h1>
        <p class="text-white-50">Preencha as informações abaixo para adicionar uma nova parada ao sistema.</p>
    </div>

    <div class="card border-0 shadow-sm" style="background-color: #13293A;">
        <div class="card-body p-4">
            <form action="{{ route('paradas.store') }}" method="POST" id="form-parada">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control bg-dark text-white border-secondary @error('par_nome') is-invalid @enderror" 
                               id="par_nome" name="par_nome" value="{{ old('par_nome') }}" placeholder="Ex: Terminal Central">
                        @error('par_nome')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control bg-dark text-white border-secondary @error('par_endereco') is-invalid @enderror" 
                               id="par_endereco" name="par_endereco" value="{{ old('par_endereco') }}" placeholder="Rua, Número, Bairro">
                        @error('par_endereco')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control bg-dark text-white border-secondary" 
                               id="par_latitude" name="par_latitude" value="{{ old('par_latitude') }}" placeholder="-23.5505">
                    </div>

                    <div class="col-md-6">
                        <label for="par_longitude" class="form-label text-warning fw-bold">Longitude (Opcional)</label>
                        <input type="text" class="form-control bg-dark text-white border-secondary" 
                               id="par_longitude" name="par_longitude" value="{{ old('par_longitude') }}" placeholder="-46.6333">
                    </div>
                </div>

                <div class="mt-4 d-flex gap-2">
                    <button type="submit" class="btn btn-warning fw-bold px-4">
                        <i class="bi bi-check-lg me-1"></i> Salvar Parada
                    </button>
