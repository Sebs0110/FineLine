            ...
            <div class="col-md-8">
                <div class="card" style="background-color: #13293A; border: none; color: white;">
                    <div class="card-header border-warning">
                        <h3 class="text-warning mb-0"><i class="bi bi-plus-circle me-2"></i>Novo Itinerário</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('itinerarios.store') }}" method="POST" id="itinerarioForm">
                            @csrf
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('iti_rota_id') is-invalid @enderror" id="iti_rota_id" name="iti_rota_id" style="background-color: #1a3a52; color: white; border-color: #2c5e86;">
                                    ...
                                    @foreach($rotas as $rota)
                                        <option value="{{ $rota->rot_id }}" {{ old('iti_rota_id') == $rota->rot_id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="time" class="form-control @error('iti_horariosaida') is-invalid @enderror" id="iti_horariosaida" name="iti_horariosaida" value="{{ old('iti_horariosaida') }}" style="background-color: #1a3a52; color: white; border-color: #2c5e86;">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('iti_diadasemana') is-invalid @enderror" id="iti_diadasemana" name="iti_diadasemana" style="background-color: #1a3a52; color: white; border-color: #2c5e86;">
                                    ...
                                    @foreach($diasSemana as $dia)
                                        <option value="{{ $dia }}" {{ old('iti_diadasemana') == $dia ? 'selected' : '' }}>{{ $dia }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="d-flex justify-content-between mt-4">
                                ...
                                <button type="submit" class="btn btn-warning" id="btnSubmit">
                                    <i class="bi bi-check-circle me-2"></i>Salvar Itinerário
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#itinerarioForm').on('submit', function() {
                $('#btnSubmit').html('<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Salvando...');
